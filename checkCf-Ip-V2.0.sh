...
busybox echo -e "<html><head><title>Gollum Scanner</title><meta name=\"viewport\" content=\"width=device-width, initial-scale=1, user-scalable=0\"/></head><body><h1>Result scan Cloudflare dari berkas $1, jika tersedia.</h1><h2><br/>Karya: Kahlil gibran<h2/>" > $cloud/hasilKeepAlive/$(basename $1).html

busybox echo -e "#!$(readlink /proc/$$/exe)\nfolder=\"\$(dirname \$(realpath \$0))\"" > $cloud/00-$(basename $0)
cat $cloud/ip2|busybox awk -v html=$1 '{print "echo \"Cek status: " $0 "\"\ncurl --connect-timeout 5 -i -X GET http://" $0 " > \$folder/hasilKeepAlive/" NR+0 ".log\nif [ ! -z \"\$(cat \"\$folder/hasilKeepAlive/" NR+0 ".log\"|busybox grep -i \"cloudflare\")\" ]\nthen\necho \"<a href=\\\"http://" $0 "\\\">Buka Tautan " $0 "</a><br/>\" >> \$folder/hasilKeepAlive/\$(basename " html ").html\nfi\n#sleep 2\nrm \"\$folder/hasilKeepAlive/" NR+0 ".log\"\n\n##"}' >> $cloud/00-$(basename $0)
